 ...
 * 2. Publishes platform packages to npm (in parallel)
...
// Read version from platform package (source of truth for binary version)
const platformPkgJson = JSON.parse(
  fs.readFileSync(
    path.join(PACKAGES_DIR, "azureauth-darwin-arm64", "package.json"),
    "utf-8",
  ),
);
const AZURE_AUTH_VERSION = platformPkgJson.version;
...
async function packagePlatform(platformConfig) {
  ...
  const gitkeepPath = path.join(binDir, ".gitkeep");
...
function npmPublish(packageDir, isScoped = false) {
  const args = ["publish"];
...
async function main() {
  ...
  // Step 2: Publish platform packages (could be parallelized but npm rate limits)
  console.log("\n" + "=".repeat(60));
